...
def extract_from_address(address):
    ...
    if not zip_match:
        print(f"❌ No ZIP found in address: {address}")
        return pd.Series(["", "", "", ""])
    ...
    print(f"Address: {address} -> Cleaned: {address_only}, ZIP: {zipcode}, City: {city}, State: {state}")
...
def main():
    ...
    for file in os.listdir(folder_name):
        if file.endswith(".xlsx"):
            ...
            try:
                ...
                new_cols_df = filtered_df["Property Address"].apply(extract_from_address).apply(pd.Series)
                new_cols_df.columns = ["State", "City", "Zip", "Clean Address"]
                new_cols_df = new_cols_df.apply(lambda col: col.map(lambda x: str(x).upper() if isinstance(x, str) else x))
                filtered_df.loc[:, ["State", "City", "Zip"]] = new_cols_df[["State", "City", "Zip"]]
    ...
    # 4. Load or create the target workbook
        ...
        try:
            os.remove(input)
            print(f"✅ Deleted availability file '{input}'")
        except Exception as e:
